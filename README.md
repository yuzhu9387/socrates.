...
**Cloud app:** [Open Socrates](https://socrates.dodofamily.com) · [Cloud deployment guide](docs/GCP.md)

**Automatic deployment:** pushes to `main` run the [Deploy Socrates workflow](https://github.com/yuzhu9387/socrates./actions/workflows/deploy.yml). It tests against disposable PostgreSQL, builds a commit-specific image, and updates Cloud Run using a keyless Google Cloud identity connection. The Cloudflare Worker for the custom hostname is deployed separately.
...
Click the avatar to open **Settings**, then choose **AI & integrations → Manage connections** to create a token:
...
The [cloud deployment guide](docs/GCP.md) covers the application in Google Cloud project `leonas-friends` and the Cloudflare Worker for the `socrates.dodofamily.com` hostname. It documents the dedicated runtime identity, Secret Manager, isolated Cloud SQL database, edge proxy, verification, and upgrades. Sharing a Cloud SQL instance with Avery does not share application accounts or sessions.
