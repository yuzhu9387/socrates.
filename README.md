...
| Settings | Avatar menu → Settings; `/#/settings` | Appearance, animation, data import/backup, account sign-out, AI connections, and activity history. |
...
### Appearance, account, and activity
...
- Review readable Activity entries, load older events, delete one entry, or clear the history.
...
Settings → **Your data** provides **Import file** and **Download backup**. **Import demo data** appears only when legacy data exists in the current browser origin. Port 4173 and port 3001 have separate browser storage; transfer content between the prototype and application using a backup file.
...
- Object APIs provide list/read/create/update/delete operations appropriate to each lifecycle. Activity is generated by operations and supports reading/deletion; it is not an arbitrary editable log.
